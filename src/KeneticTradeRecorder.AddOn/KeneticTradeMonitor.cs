...
{
    /// <summary>
    /// Phase 1 trade observation engine. Records every execution and the assembled
    /// trade lifecycle (entry / scale-in / scale-out / exit / reversal) for the
    /// selected accounts, without modifying any trading behavior.
    ///
    /// This is a plain class, NOT an AddOnBase. NinjaTrader only activates AddOnBase
    /// types that it compiles from source, so a source shim (KeneticRecorderAddOn,
    /// deployed to bin\Custom\AddOns) is what NinjaTrader instantiates; that shim
    /// calls Start()/Stop() on the singleton Instance here. The heavy logic stays in
    /// this net48 library (modern C#); the shim stays trivial (NinjaScript-compiled).
    /// </summary>
    public sealed class KeneticRecorderHost
    {
        /// <summary>Process-wide singleton the NinjaScript shim drives. Constructed on first use.</summary>
        public static readonly KeneticRecorderHost Instance = new KeneticRecorderHost();

        /// <summary>Begin recording: load config, subscribe accounts, start the status writer.</summary>
        public void Start()
        {
        ...
        /// <summary>Stop recording and release resources (called on NinjaTrader shutdown).</summary>
        public void Stop()
        {
            UnhookAccounts();
            _sink?.Dispose();
